...
namespace StockTrackerService.Extensions
{
  public static class AppConfigKeysExtensions
  {
    ...
    public static string GetValue(this AppConfigKeys key)
      => ConfigurationManager.AppSettings[key.GetName()] ?? string.Empty;
    ...
    public static long GetAsLong(this AppConfigKeys appKey)
    {
      try
      {
        if (!long.TryParse(appKey.GetValue(), out var setting))
          throw new ArgumentOutOfRangeException(appKey.GetName());
